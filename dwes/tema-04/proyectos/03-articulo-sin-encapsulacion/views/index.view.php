                ...
                <table class="table table-striped table-hover align-middle shadow-sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?= implode(', ', Class_tabla_articulos::categorias_a_nombres($articulo->categorias)) ?></td>
                            <td><?= htmlspecialchars($articulo->unidades) ?></td>
                            <td class="text-end"><?= number_format($articulo->precio, 2, ',', '.') ?></td>
                            <td class="text-end">
                                <a href="edit.php?id=<?= $articulo->id ?>" class="btn btn-sm btn-primary me-1">
                                    <i class="bi bi-pencil-square"></i> Editar
                                </a>
                                <a href="delete.php?id=<?= $articulo->id ?>" class="btn btn-sm btn-danger" onclick="return confirm('¿Estás seguro de que deseas eliminar este artículo?');">
                                    <i class="bi bi-trash"></i> Eliminar
                                </a>
                                <a href="show.php?id=<?= $articulo->id ?>" class="btn btn-sm btn-info">
